...
import { Map, FileEdit, Loader2, CheckCircle2, XCircle, RefreshCw } from 'lucide-react';
...
export default function AdminDashboard() {
    ...
    const [relinkStatus, setRelinkStatus] = useState({}); // { [areaId]: 'loading' | 'success' | 'error' }
    ...
    // Per-card rebuild: re-runs the link engine for this one area only.
    const handleRelinkArea = async (area) => {
        setRelinkStatus(prev => ({ ...prev, [area.id]: 'loading' }));
        try {
            const res = await fetch(`/api/areas/${area.id}/relink`, { method: 'POST' });
            const data = await res.json();
            setRelinkStatus(prev => ({ ...prev, [area.id]: data.success ? 'success' : 'error' }));
            if (data.success) {
                fetchAreas();
            } else {
            ...
            setRelinkStatus(prev => ({ ...prev, [area.id]: 'error' }));
            alert(err.message);
        }
        setTimeout(() => {
            setRelinkStatus(prev => ({ ...prev, [area.id]: null }));
        }, 3000);
    };
            ...
            <div className="admin-main">
                ...
                <header className="admin-header">
                    ...
                    <div className="admin-header-actions">
                        ...
                        <Link prefetch={false} href="/admin/seo" className="btn btn-secondary">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <Link prefetch={false} href="/admin/indexing" className="btn btn-secondary">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                <path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"></path>
                            </svg>
                            Indexing
                        </Link>
                        <Link prefetch={false} href="/admin/content-audit" className="btn btn-secondary">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <Link prefetch={false} href="/admin/settings" className="btn btn-secondary">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {hasDraft && (
                    <div className="draft-banner">
                        ...
                        <div className="draft-actions">
                            <Link prefetch={false} href="/admin/areas/add" className="btn-continue-draft">
                                Continue Editing
                            </Link>
                ...
                <div className="admin-controls">
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <Link prefetch={false} href="/admin/areas/add" className="btn btn-accent">
                            Add New Area
                        </Link>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="empty-state">
                        ...
                        {!search && (
                            <Link prefetch={false} href="/admin/areas/add" className="btn btn-primary">
                                Add Your First Area
                            </Link>
                        )}
                    </div>
